...
#' 
#'
#' @param cov_config Covariate config that specifies four things: The MBG standard covariates names (list found here: URLPATH); 
...
#'                   and the covariate release date e.g., "DATE" (if this is an empty or missing column it will substitute most current covariate release). 
...
#' cov_config <- data.table(covariate = "worldpop", measure = "total", agg_method = "sum", release = "DATE")
...
frac_agg_covs <- function(cov_config, 
  ...
  if (is.null(custom_covariate)) {
    ...
    if (nlayers(custom_covariate) != length(years)) {
  ...
  link_table_list <- build_link_table(shapefile_version = shapefile_version,
                                      cores = cores,
                                      custom_shapefile_path = NULL,
                                      custom_shapefile_field = 'ADM2_CODE',
                                      region=reg)
  ...
  codes <- get_adm0_codes(reg, shapefile_version = shapefile_version)
  subset_shape <-  polys[polys$ADM0_CODE %in% codes,]
  remove(polys)
  simple_raster <- build_simple_raster_pop(subset_shape, 
                                           field = 'ADM2_CODE', 
                                           link_table = link_table, 
                                           id_raster = id_raster)[['simple_raster']]
  ...
  if (is.null(custom_covariate)) {
    ...
    message("Loading custom covariates from custom_covariate argument")
    cov_rasters <- list(custom_covariate)
  }
