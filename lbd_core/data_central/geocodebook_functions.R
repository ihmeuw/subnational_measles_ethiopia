...
#' @description Pull geocodebooks from database and csvs on J: drive
#' @author USERNAME
#'
...
get_geocodebooks <- function(nids = NULL, keep_all_columns = F, pull_all_nids = F){
  ...
  j <- ifelse(Sys.info()[1]=="Windows", "FILEPATH", "FILEPATH")
  
  #if RPostgres isn't loaded in, try to load
  if(!("RPostgreSQL" %in% loadedNamespaces())){
    tryCatch(
      {library(RPostgreSQL)},
      error = function(err) {
        tryCatch(
          {library(RPostgreSQL, lib.loc=paste0("FILEPATH"))},
          error = function(e) {
  ...
  if(length(nids_not_in_db) != 0) {
    ...
    #match csv columns and datatypes to db table
  ...
  setorder(combined_table, "nid", "geospatial_id")
...
get_geocodebook_db <- function(nids, pull_all_nids = F) {
  ...
  con <- tryCatch({
           con <- dbConnect(dbDriver("PostgreSQL"), dbname = "postgres",
                            host = "lbd-psql-db-p01.db.ihme.washington.edu", port = 5432,
                            user = "dbview", password = "password")}, 
           error = function(err) {
  ...
  if(nrow(db_result_dt) == 0){
    ...
    db_result_dt <- setNames(data.table(matrix(nrow = 0, ncol = length(column_names))), column_names)
  ...
  db_result_dt[, c("long", "lat") := tstrsplit(snapped_geocoord, " ")]
  db_result_dt[, c("original_long", "original_lat") := tstrsplit(original_geocoord, " ")]
  db_result_dt[, long := gsub("POINT\\(", "", long)]
  db_result_dt[, original_long := gsub("POINT\\(", "", original_long)]
  db_result_dt[, lat := gsub("\\)", "", lat)]
  db_result_dt[, original_lat := gsub("\\)", "", original_lat)]
  ...
  #Outer merge on point/poly tables adds in NA nids for geographies that have been retired (when pulling all nids)
  db_result_dt <- db_result_dt[!is.na(nid),]
