...
if(sys == 'local'){
  ...
  package_list <- c(t(read.csv(paste('FILEPATH'), header = FALSE)))
...
saveRDS(dat, file = sprintf('FILEPATH', date))
...
train.date <- 'DATE'
train.dat <- readRDS(sprintf('FILEPATH', train.date))
...
test.date <- '20181022'
test.dat <- readRDS(sprintf('FILEPATH', test.date))
