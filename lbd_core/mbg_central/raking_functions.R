...
#' @param crosswalk Boolean default `T`, for models run before the new gaul shapefiles ready on DATE, the cell_pred needs to be crosswalked to match the subnational raking raster. Introduces NAs into raster where the new simple raster has values but the original does not. 
...
#' @param if_no_gbd default `return_na`, other option `return_unraked`. If return_na, any location-years without gbd estimates will return NA raking factors. if return_unraked, will any location-years without gbd estimates will return 1 for linear raking, 0 for logit raking.
#' 
#' @import rgdal, data.table, sp, rgeos, raster, data.table, foreign
#'
...
rake_cell_pred <- function(cell_pred, 
  ...
  pop_raster_annual  <- extend(pop_raster_annual, new_simple_raster, values=NA)
  pop_raster_annual  <- crop(pop_raster_annual, extent(new_simple_raster))
  pop_raster_annual  <- setExtent(pop_raster_annual, new_simple_raster)
  pop_raster_annual  <- mask(pop_raster_annual, new_simple_raster)
  ...
  outputlist <- list("raked_cell_pred" = raked_cell_pred,
...
make_custom_raking_shapefile <- function(countries,
  ...
  if (!all(subnational_countries %in% subnational_iso3s$iso3)) {
    ...
    message("All subnationals are included in admin shapefiles from DATE or later")
  }
...
calculate_raking_factors = function(cell_pred, 
  ...
  if(reg=="vax_wssa"){sr_locs <- c(200,201,202,203,204,205,206,207,208,209,210,211,212,213,214,215,216,217,218)}
  if(reg=="vax_seas"){sr_locs <- c(10,12,15,19,18,26,20,13,16,4709,4710,4711,4712,4713,4714,4715,4716,4717,4718,4719,4720,4721,4722,4723,4724,
                                   4725,4726,4727,4728,4729,4730,4731,4732,4733,4734,4735,4736,4737,4738,4739,4740,4741,4742)}
  if(reg=="vax_soas"){sr_locs <- c(161, 162,17,164,165,44538,4841,4842,4843,4844,4846,4849,4850,4851,4852,4853,4854,4855,4856,4857,4859,4860,
                                   4861,4862,4863,4864,4865,4867,4868,4869,4870,4871,4872,4873,4874,4875)}
  ...
  # drop NGA GBD subnationals
  nga_list <- c(25351, 25351, 25325, 25353, 25338,
                25339, 25354, 25335, 25337, 25322, 25336, 25344, 25333,
                25319, 25349, 25341, 25352, 25332, 25343, 25348, 25340,
                25324, 25347, 25330, 25345, 25346, 25329, 25331, 25326, 25328, 25321, 25342,
                25327, 25318, 25334, 25350, 25320, 25323)
  raker$loc[raker$loc %in% nga_list] <- 214

  # drop PAK GBD subnationals
  pak_list <- c(53615,53616,53617,53618,53619,53620,53621)  
  raker$loc[raker$loc %in% pak_list] <- 165

  # drop PHL GBD subnationals
  phl_list <- c(53533,53534,53535,53536,53537,53538,53539,53540,
                53541,53542,53543,53544,53545,53546,53547,53548,
                53549,53550,53551,53552,53553,53554,53555,53556,
                53557,53558,53559,53560,53561,53562,53563,53564,
                53565,53566,53567,53568,53569,53570,53571,53572,
                53573,53574,53575,53576,53577,53578,53579,53580,
                53581,53582,53583,53584,53585,53586,53587,53588,
                53589,53590,53591,53592,53593,53594,53595,53596,
                53597,53598,53599,53600,53601,53602,53603,53604,
                53605,53606,53607,53608,53609,53610,53611,53612,
                53613,53614)
  raker$loc[raker$loc %in% phl_list] <- 16
...
apply_raking_factors = function(cell_pred, 
  ...
  if(reg=="vax_wssa"){thelocs <- c(200,201,202,203,204,205,206,207,208,209,210,211,212,213,214,215,216,217,218)}
  if(reg=="vax_seas"){thelocs <- c(10,12,15,19,18,26,20,13,16,4709,4710,4711,4712,4713,4714,4715,4716,4717,4718,4719,4720,4721,4722,4723,4724,
                                   4725,4726,4727,4728,4729,4730,4731,4732,4733,4734,4735,4736,4737,4738,4739,4740,4741,4742)}
  if(reg=="vax_soas"){thelocs <- c(161, 162,17,164,165,44538,4841,4842,4843,4844,4846,4849,4850,4851,4852,4853,4854,4855,4856,4857,4859,4860,
                                   4861,4862,4863,4864,4865,4867,4868,4869,4870,4871,4872,4873,4874,4875)}
  ...
  nga_list <- c(25351, 25351, 25325, 25353, 25338,
                ...
                25324, 25347, 25330, 25345, 25346, 25329, 25331, 25326, 25328, 25321, 25342,
                25327, 25318, 25334, 25350, 25320, 25323)
  dt$loc[dt$loc %in% nga_list] <- 214

  # drop PAK GBD subnationals
  pak_list <- c(53615,53616,53617,53618,53619,53620,53621)  
  dt$loc[dt$loc %in% pak_list] <- 165
