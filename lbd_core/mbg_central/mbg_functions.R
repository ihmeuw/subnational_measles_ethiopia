...
save_mbg_input <- function(indicator = indicator, indicator_group = indicator_group, df = df , simple_raster = simple_raster, simple_raster2 = NULL, mesh_s = mesh_s,
  ...
  save(list = to_save, file = paste0('FILEPATH'))
  message(paste0('All inputs saved to FILEPATH'))
...
fit_mbg <- function(indicator_family,
  ...
  inla_working_dir <- paste0("FILEPATH")
...
predict_mbg <- function(res_fit, cs_df, mesh_s, mesh_t, cov_list,
  ...
  #   cell_adm2_res <- matrix(0, ncol = ncol(cell_l), nrow = nrow(cell_l))
  ...
  #   adm2_gauls.with.res <- as.numeric(rownames(pred_adm2_res))
  #   adm2_gaul.vec <- values(simple_raster2)[adm2_cell_idx]
  #   adm2_gaul.t.vec <- rep(adm2_gaul.vec, nperiod) ## expand gaul vec in time
  ...
  #   for (gsubnat in adm2_gauls.with.res) {
      
  #     ## Find the rows in simple_raster2 corresponding to the iterating subnat
  #     gsubnat.rows <- which(adm2_gaul.t.vec == gsubnat)
  #     gg.idx <- which(adm2_gauls.with.res == gsubnat)
      
  #     if (gsubnat %in% adm2_gauls.with.res) {
      
  #     message(paste0("adding RE for adm2 ", gsubnat))
      
  #     cell_adm2_res[gsubnat.rows, ] <- cell_adm2_res[gsubnat.rows, ] + 
  #       matrix(rep(pred_adm2_res[gg.idx, ], length(gsubnat.rows)), ncol = ncol(cell_adm2_res), byrow = TRUE)
      
  #     } else {

  #       ## this subnat unit had no data and we need to take a draw from the RE dist for draw in preds
  #       ## for draw, we draw one value of the random effect using pred_subnat_prec
  #       re.draws <- rnorm(n = ncol(cell_adm2_res), mean = 0, sd = 1 / sqrt(pred_adm2_prec))
        
  #       ## replicate by all rows in that country and add it on
  #       cell_adm2_res[gsubnat.rows, ] <- cell_adm2_res[gsubnat.rows, ] + 
  #         matrix(rep(re.draws, length(gsubnat.rows)), ncol = ncol(cell_adm2_res), byrow = TRUE)
  #     }
  #   }
  # }
...
save_mbg_preds <- function(config, time_stamp, run_date, mean_ras, sd_ras, res_fit, cell_pred, df ,pathaddin="") {
  ...
  write.csv(config, paste0('FILEPATH'), row.names = FALSE)
