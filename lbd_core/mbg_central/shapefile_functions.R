...
#'
#' @param version admin shapefile version to pull e.g., 'DATE'. Default: 'current'
...
is_admin_shapefile_string <- function(s) {
  if (s == "current") {
      ...
      TRUE # "DATE" or another admin shapefile release date
  } else {
    FALSE
  }
}
...
detect_adm_shapefile_date_type <- function(shpfile_path = get_admin_shapefile(version = modeling_shapefile_version)){
  ...
  gadm.exceptions <- c('DATE') ## dates before transition.date are actually GADMw
