...
## data <- df
## shp_full <- plot_shp <- shapefile('FILEPATH')
## xy <- data[,c('long', 'lat')]
## ss <- data$exposed
## ts <- 1e6
...
## shp_full <- shapefile('FILEPATH')
## folds <- ad2_folds(admin_raster='FILEPATH',
##                   shape_ident="gaul_code",
##                   admin_shps='FILEPATH',
##                   data=df,
##                   strat_cols=NULL,
...
make_folds <- function(data,
                       ...
                       temp_strat = NULL,
                       spte_strat = NULL,
                       save.file = paste0('FILEPATH'), ## place to save the final object from the function
                       ...,
                          ...
                          function(x){
                            ...
                            lapply(1:max(stratum[[x]][['t_fold']]),
                                   function(y){
                                     ...
                                     ## make a mask for ther region we care about
                                     mask <- rasterize_check_coverage(mask_shape, raster(mask_raster), field=names(mask_shape)[1])*0
...
rename_USERNAME_shapefiles<-function(len=(length(Regions)*4),prefix){
  for(i in 1:len)
