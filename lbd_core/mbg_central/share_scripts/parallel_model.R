...
message("Printing git hash for 'core_repo' and checking against LBD Core Code main repo")
...
#Load USERNAME pardiso license. Note: will only work for USERNAME
...
## ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
PID <- Sys.getpid()
tic("Entire script") # Start main timer
...
if(as.logical(skiptoinla) == FALSE){
  ...
  if(holdout==0) {
    ...
    if(!exists("loadinputdata_from_rundate")) {
      ...
      #!#!#!#!#!#!#!#
      #Keep age bins in range 2:7, but then converts values down to 1:6 for better indexing during modeling
      df[,original_age:=age_bin]
      df<-df[age_bin<8]
      df<-df[age_bin>1]
      df[,age_bin:=age_bin-1]
      df[age_bin_agg=='1:3', age_bin_agg:='2:3']  #Highly suspect, temporary adjustment
      df[age_bin_agg=='1:4', age_bin_agg:='2:4']  #Highly suspect, temporary adjustment
      #!#!#!#!#!#!#!#!#!#!
      ...
      load(paste0('FILEPATH'))
      save(df, file=paste0("FILEPATH")) # save to current rundate dir
    }
  } else {
    ...
    stratum_ho <- readRDS(paste0('FILEPATH'))
  ...
  if(indicator == 'mcv1_cov'|indicator == 'mcv1_disagg'){
    try(reg_opt <- fread(paste0('FILEPATH')))
  }
  if(indicator == 'dpt3_timeliness_ratio'){
    reg_opt <- fread(paste0('FILEPATH'))
  }
  if(indicator == 'polio3_cov'){
    reg_opt <- fread(paste0('FILEPATH'), na.strings="")
  }
  ...
  if(!(exists('reg_opt'))) reg_opt <- fread(paste0('FILEPATH'))
  ...
  if (nchar(gbd_fixed_effects) > 0) {
    ...
    #reg_opt <- fread(paste0('FILEPATH'))
    #reg_opt <- reg_opt[which(reg_opt$gbd=='T'),]
  ...
  tic("Stacking - all") ## Start stacking main timer
  ...
  toc(log = T) ## End stacking main timer
  ...
  file.copy(from = paste0('FILEPATH'),
            to = paste0('FILEPATH'))
...
tic("MBG - all") ## Start MBG main timer
...
if(fit_with_tmb == FALSE) {
  ...
  #Temporary setup for implementing ANC FE and no other FEs
  #  if((!as.logical(use_stacking_covs) & !as.logical(use_raw_covs))|as.logical(use_raw_covs)) {
  #    covs_list <- cov_list
  #  } else if(as.logical(use_stacking_covs)) {
  #    covs_list <- stacked_rasters
  #  } else {
  #    covs_list <- NULL
  #  }
