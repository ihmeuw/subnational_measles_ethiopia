...
get_population_data     <- function(simple_raster){
  ...
  pop<- brick(sprintf('FILEPATH',root))
...
load_cell_preds <- function(indicator_group,
  ...
  if(u5m){
    if(ageasindic==FALSE){
      load(paste0('FILEPATH')) # the 0 are no holdout
    } else {
      load(paste0('FILEPATH'))
    }
    cell_pred <- cptmp
  } else {
    load(paste0('FILEPATH')) # the 0 are no holdouts
  }
  return(cell_pred)
}
...
save_best_model <- function(indicator_group, indicator, run_date, measure) {
  ...
  raked <- brick(paste0("FILEPATH"))
  unraked <- brick(paste0("FILEPATH"))
...
compile_results_table <- function(indicator_group,
  ...
  ## Save copy of all_admins (by level) 
  for(adm in c(0,1,2)) {
    if(adm == 0) {
...
get_weighted_correlation_raw_estimated<-function(indicator_group,
  ...
  results_tif<-raster::stack(paste0("FILEPATH"))
...
post_load_combine_save <- function(regions = strata,
  ...
  if ("unraked" %in% raked) {
    lookup_dir <- paste0(sprintf("FILEPATH"))
...
#' @description Access pre-saved RDS population outputs from FHS (saved out in \code{FILEPATH})
#' 
#' @param population_version A version tag found from looking into: \code{FILEPATH}
...
#' 
#' @export
get_fhs_population <- function(population_version = "DATE",
                               pop_measure = "a0004t",
                               sex_ids = 3,
                               scenarios = 0,
                               year_ids = NULL,
  ...
  ## Get pop data
  pop_data <- readRDS(paste0("FILEPATH"))
