...
#
# For additional questions ask USERNAME
...
fractionally_rake_counts <- function(count_cell_pred,
  ...
  year <- rep(year_list, each=(nrow(cell_pred)/length(year_list)))
...
#' @param shapefile_version character. The release date of the admin shapefile to use e.g., "DATE".
#'
...
get_link_table <- function(simple_raster, shapefile_version) {
  #read in link_table
  global_link <- data.table(readRDS(sprintf("FILEPATH", shapefile_version)))
  #read in id_raster
  global_raster <- readRDS(sprintf("FILEPATH", shapefile_version))
...
#' @examples
#' \dontrun{
#' link <- build_link_table(shapefile_version = "DATE", cores = 10, region = "stage1+stage2")
#'
#' link <- build_link_table(shapefile_version = NULL, cores = 10, region, region = NULL, custom_shapefile_path = "FILEPATH", custom_shapefile_field = "ADM_CODE")
#' }
#'
#' @export
build_link_table <- function(shapefile_version,
  ...
  main_pop <- raster::brick('FILEPATH')
  cropped_pop <- raster::crop(main_pop, polys, snap = 'out')
  ...
  get_intersect <- function(poly) {
    ...
    intersects <- suppressWarnings(sf::st_intersects(poly, target)) # returns sparse matrix
    ...
    intersection <- suppressWarnings(sf::st_intersection(poly, target))
...
#' They do not directly support a counts cell_pred (see USERNAME's work on 
...
#' @param sharedir sharedir       <- sprintf('FILEPATH',indicator_group,indicator)
...
#' @param interval_mo the time in months between the modeled years
#' @param rake_subnational a logical value indicating the use of subnational raking targets or not
#' @param sharedir Path to sharedir e.g.: \code{sharedir <- sprintf('FILEPATH')}
#' @param run_date model run date string
#' @param indicator modeled indicator
#' @param main_dir Path to main_dir e.g.: \code{main_dir <- paste0('FILEPATH')}
...
load_populations_cov <- function(reg,
  ...
  if (measure == 'prevalence') {
    covs = fetch_from_rdata(paste0('FILEPATH'), 'cov_list')
    fes = fetch_from_rdata(paste0('FILEPATH'), 'all_fixed_effects')
...
#' get_fractional_rf function above (see USERNAME's work).
#'
