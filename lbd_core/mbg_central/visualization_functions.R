...
subnational_ts_plots <- function(ad0_df,
  ...
  simplify_spdf <- function(the_spdf, tol, ...) {
  #  simple_spdf <- gSimplify(the_spdf, topologyPreserve = T, ...)
  
    simple_spdf <- ms_simplify(input=the_spdf,keep=tol, keep_shapes=TRUE)
      return(simple_spdf)
  }
  ...
  if (is.null(ad0_shape)){
    ...
    ad0_shape_simple <- subset(ad0_shape, ADM0_CODE %in% subset_codes)
  }
  ...
  if (is.null(ad1_shape)){
    ...
    ad1_shape_simple <- subset(ad1_shape, ADM0_CODE %in% subset_codes)
  }
  ...
  if (is.null(ad2_shape)){
    ...
    ad2_shape_simple <- subset(ad2_shape, ADM0_CODE %in% subset_codes)
  }
  ...
  ihme_logo <- png::readPNG('FILEPATH')
  ...
  if ("ad0" %in% plot_levels) {
    ...
    for (i in 1:length(ad0_code_list)) {
      ...
      plot_df <- 
        ad0_df_reg %>% filter(year == max(year_list) & run=='raked') %>% dplyr::select(ADM0_CODE, ihme_lc_id, mean)
      ...
      gg_lastyear_map <- last_year_map(ad0_shape_reg_df, centroids, admin = 0, val_range, distiller_direction, max(year_list))
      ...
      if (plot_data == T & multiple_runs == F) main_plot <- arrangeGrob(gg_ad0_ts, gg_locator_map, title_grob, gg_lastyear_map, layout_matrix = lay)
      if (plot_data == T & multiple_runs == T) main_plot <- arrangeGrob(gg_ad0_ts, gg_locator_map, title_grob, gg_lastyear_map, ihme_grob_multiple, layout_matrix = lay)
      if (plot_data == F) main_plot <- arrangeGrob(gg_ad0_ts, gg_locator_map, title_grob, gg_lastyear_map, ihme_grob,
  ...
  if ("ad1" %in% plot_levels) {
    ...
    for (ctry in sort(countries)) {
      ...
      plot_df <- 
        ad1_df_ctry %>% filter(year == max(year_list) & run=='raked') %>% dplyr::select(ADM1_CODE, mean)
      
      # Merge and fortify outside of ggplot to speed things up
      plot_df <- plot_df %>% distinct() %>% data.table()
      ad1_national <- suppressWarnings(merge(ad1_national, plot_df))
      ...
      gg_ad1_map <- last_year_map(ad1_national_df, centroids, admin = 1, val_range, distiller_direction, max(year_list))
      ...
      # Create the overall plot by arranging the grobs
      lay <- plot_overlay(plot_data, multiple_runs)
      if (plot_data == T & multiple_runs == F) main_plot <- arrangeGrob(gg_ad1ts, gg_locator_map, title_grob, gg_ad1_map, layout_matrix = lay)
      if (plot_data == T & multiple_runs == T) main_plot <- arrangeGrob(gg_ad1ts, gg_locator_map, title_grob, gg_ad1_map, ihme_grob_multiple, layout_matrix = lay)
      if (plot_data == F) main_plot <- arrangeGrob(gg_ad1ts, gg_locator_map, title_grob, gg_ad1_map, ihme_grob,
                                                     layout_matrix = lay, heights = c(1,1,1,1,1,0.6))
      grid.draw(main_plot)
  ...
  if ("ad2" %in% plot_levels) {
    ...
    for (ctry in sort(countries)) {
      ...
      for (ad1_code in ad1_table$ADM1_CODE) {
        ...
        plot_df <- 
          ad2_df_ad1 %>% filter(year == max(year_list) & run=='raked') %>% dplyr::select(ADM2_CODE, mean)
        
        # Merge and fortify outside of ggplot to speed things up
        plot_df <- plot_df %>% distinct() %>% data.table()
        ad2_ad1 <- suppressWarnings(merge(ad2_ad1, plot_df, duplicateGeoms = T))
        ...
        # Prepare for plotting
        gg_ad2_map <- last_year_map(ad2_ad1_df, centroids, admin = 2, val_range, distiller_direction, max(year_list))
        ...
        # Create the overall plot by arranging the grobs
        lay <- plot_overlay(plot_data, multiple_runs)
        if (plot_data == T & multiple_runs == F) main_plot <- arrangeGrob(gg_ad2ts, gg_locator_map, title_grob, gg_ad2_map, layout_matrix = lay)
        if (plot_data == T & multiple_runs == T) main_plot <- arrangeGrob(gg_ad2ts, gg_locator_map, title_grob, gg_ad2_map, ihme_grob_multiple, layout_matrix = lay)
        if (plot_data == F) main_plot <- arrangeGrob(gg_ad2ts, gg_locator_map, title_grob, gg_ad2_map, ihme_grob,
                                                       layout_matrix = lay, heights = c(1,1,1,1,1,0.6))
