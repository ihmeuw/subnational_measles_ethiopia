...
rm(list=ls())
root <- ifelse(Sys.info()[1]=="Windows", "FILEPATH", "FILEPATH") #discovering what system the code is running on-- the cluster, windows, etc.
n_draws<-1000 # 
...
results<-fread(paste0("FILEPATH"))
results_tables_path<-paste0("FILEPATH")
diffs_tables_path<-paste0("FILEPATH")
results_time_series_path<-paste0("FILEPATH")
...
admin_level<-2
sp_h<-data.table(foreign::read.dbf(paste0("FILEPATH")))
