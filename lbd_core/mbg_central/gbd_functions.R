...
get_con <- function(dbname, host) {
  con <- suppressWarnings(src_mysql(dbname = dbname, host = host, user = "dbview", password = "PASSWORD"))
  return(con)
}
...
get_cov_estimates <- function(covariate_name_short, filters=list()) {
  ...
  conn <- dbConnect(RMySQL::MySQL(), host="URLPATH", username="readonly", password="PASSWORD")
