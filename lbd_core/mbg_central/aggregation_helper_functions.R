...
read_z_mapping <- function(z_map_file, type="age"#, raked_worldpop=F
                           ) {
...
process_input_data <- function(df, 
  ...
  uncollapsed_df <- lapply(1:nrow(df), function(i) {
    ...
    idx_small_aw <- agg_weight < 1/(low_weight_multiplier*length(agg_weight))
    if(any(idx_small_aw)){
      agg_df <-as.data.table(agg_weight)
      names(agg_df) <- 'wt'
      agg_df[, ord := 1:nrow(agg_df)]
      setorder(agg_df, wt)
      x=0
      agg_df[, idx_small_aw := FALSE]
      for(i in 1:nrow(agg_df)){
        x = x + agg_df[i]$wt
        if(x > 0.01 | agg_df[i]$wt >= 1/(low_weight_multiplier*nrow(agg_df))) break() else agg_df[i, idx_small_aw :=TRUE]
      }
      setorder(agg_df, ord)
      message(paste0('dropping ', length(agg_weight[agg_df$idx_small_aw]), ' rows of ', length(agg_weight)))
      agg_weight <- agg_weight[!agg_df$idx_small_aw]
      agg_weight <- agg_weight/sum(agg_weight) # rescale so it sums to 1
      idx_small_aw <- agg_df$idx_small_aw
    }
    if(!is.na(df_row$latitude)) {
      latitude <- df_row$latitude
      longitude <- df_row$longitude
    } else {
      latitude <- df_row$coordinates.lat[[1]][idx_nonzero_aw][!idx_small_aw]
      longitude <- df_row$coordinates.long[[1]][idx_nonzero_aw][!idx_small_aw]
    }
    if(!is.na(df_row$z_ag)) {
      z <- df_row$z[[1]][idx_nonzero_aw][!idx_small_aw]
    } else {
      if(zcol != "z_column_default_blank") {
        z <- df_row$z_tmp
      } else {
    ...
    first_entry <- c(1, rep(0, sum(!idx_small_aw)-1)) # useful for not plotting duplicates
    cbind(df_row[,..cols_include], longitude, latitude, 
          z, agg_weight, first_entry, to_drop)
  }
}) 
    ...
    for(year_needed in unique(age_table[z_ag == ages & country == reg]$year)) {
      worldpop_cov_rasters <- lapply(ages_needed, function(age) {
        suppressMessages(load_worldpop_covariate(simple_polygon,
                                ...
                                pop_measure = paste0(z_map$value[z_map$z == age]),
...
#' years, and z_ag.
#' @param ref_raster The main raster.
        ...
        for(year_needed in years) {
          ...
          worldpop_cov_rasters <- lapply(ages_needed, function(age) {
            suppressMessages(load_worldpop_covariate(sub_raster,
                                                    covariate = 'worldpop',
                                                    pop_measure = ifelse(is.na(age), "total", paste0(z_map$value[z_map$z == age])),
                                                    pop_release = pop_release,
                                                    start_year = year_needed,
                                                    end_year = year_needed,
                                                    interval = as.numeric(interval_mo))$worldpop)
...
#' @param sub_raster The main raster cropped to the polygon of interest.
#' @param disaggregation_factor Integer. The number of cells the raster corresponding
#'   to a polygon should be broken into. This can be useful if the polygon is very
#'   small in area.
#' @param auto_disaggregate Logical. Should a disaggregation_factor of 5 (i.e. breaking
