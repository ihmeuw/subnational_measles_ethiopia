...
report_each_gaul <- function(gaul, quilt_plot_list, target_plot_list, shapefile_version = 'current') {
  ...
  if(indicator == 'edu_mean') {
    ...
    load('FILEPATH')
    dhs_admin1_data <- fread('FILEPATH')
...
extract_raked_and_unraked <- function(gaul,
  ...
  if(load_simple_raster==FALSE) {
    simple_raster <- main_list[[paste0('list_', gaul, '.simple_', gaul)]]
  }
...
summarize_admin2 <- function(gaul, indicator, indicator_group, run_date, nperiod, main_list) {
  ...
  ## Get admin2 codes
    country_admin2 <- main_list[[paste0('list_', gaul, '.admin2_', gaul)]]
    country_admin2 <- crop(country_admin2, extent(main_list[[paste0('list_', gaul, '.simple_', gaul)]]))
    country_admin2 <- setExtent(country_admin2, main_list[[paste0('list_', gaul, '.simple_', gaul)]])
    country_admin2 <- mask(country_admin2, main_list[[paste0('list_', gaul, '.simple_', gaul)]])

  ## Get ids for all cells
    cell_idx <- seegSDM:::notMissingIdx(main_list[[paste0('list_', gaul, '.simple_', gaul)]])
...
raking_factors_map <- function(indicator,
  ...
  # extract admin0
  main_shape <- readOGR(dsn='FILEPATH', layer='africa_ad0')
  names(main_shape)[names(main_shape)=="ADM0_CODE"] <- "GAUL_CODE"
  gaul_to_loc_id <- get_location_code_mapping(shapefile_version = shapefile_version)
  main_shape <- merge(main_shape, gaul_to_loc_id, by="GAUL_CODE")
  subset_shape <- main_shape[main_shape@data$GAUL_CODE %in% gaul, ]
  admin0.dt <- data.table(fortify(subset_shape))

  rf <- fread(paste0('FILEPATH'))
...
is_oos_preds <- function(rd = run_date,
  ...
  output.dir <- sprintf("FILEPATH")

  ## Load data
  datdir <- sprintf('FILEPATH')
...
compare_admin_estimates <- function(nid_list, results_raster,
  ...
  compile_polygons <- function(this_nid, estimates_raster) {
    ...
    country_pops <- mask(country_pops, main_list[[paste0('list_', gaul, '.simple_', gaul)]])
    message('crop')
    country_estimates <- crop(estimates_raster, extent(main_list[[paste0('list_', gaul, '.simple_', gaul)]]))
    country_estimates <- setExtent(country_estimates, main_list[[paste0('list_', gaul, '.simple_', gaul)]])
    country_estimates <- mask(country_estimates, main_list[[paste0('list_', gaul, '.simple_', gaul)]])
...
plot_target <- function(gaul_list, df, sample, subset_shape, target, target_type, region=NULL, shapfile_version = 'current') {
  ...
  # based on plot_quilt() by USERNAME
