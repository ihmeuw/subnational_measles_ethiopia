...
load_config <- function(repo, indicator_group, indicator, config_name=NULL, covs_name = NULL, post_est_only=FALSE, run_date = '') {
  ...
  if (is.null(config_name)) {
    ...
    if(post_est_only==FALSE) config <- fread(paste0('FILEPATH'), header=FALSE)
    ## If running analysis on existing model, use config from that model's outputs folder
    if(post_est_only==TRUE) config <- fread(paste0('FILEPATH'))
  } else {
...
## Make template rasters (pull from central analysis folders managed by USERNAME based off the area to model specified in config)
#   Arguments:
...
get_template_raster <- function(simple) {
  ...
  analysis_raster_dir <- paste0('FILEPATH')
...
load_input_data <- function(indicator, agebin = 0, removeyemen = FALSE, pathaddin = "",
  ...
  if(withdate)             filename <- paste0('FILEPATH')
  ...
  if(update_run_date == TRUE) {
    if(dir.exists(paste0('FILEPATH')) == TRUE) {
      existing_dir <- paste0('FILEPATH')
    ...
    if(dir.exists(paste0('FILEPATH')) == FALSE) {
      run_date_dir <- paste0('FILEPATH')
...
load_simple_polygon <- function(gaul_list, buffer, tolerance = 0.2,
  ...
  if (is.null(custom_shapefile_path) & is.null(custom_shapefile)) {
    message("Opening main shapefile...")
    main_shape <- readOGR(get_admin_shapefile(admin_level = 0, raking = raking,
                                                version = shapefile_version))
    main_shape@data$ADM0_CODE <- as.numeric(as.character(main_shape@data$ADM0_CODE))
    subset_shape <- main_shape[main_shape@data$ADM0_CODE %in% gaul_list, ]
  } else if (!is.null(custom_shapefile_path) & is.null(custom_shapefile)) {
    message("Opening custom shapefile...")
    main_shape <- readOGR(custom_shapefile_path)
    subset_shape <- main_shape
  } else if (is.null(custom_shapefile_path) & !is.null(custom_shapefile)) {
    main_shape <- custom_shapefile
    subset_shape <- main_shape
  }
  ...
  if(subset_only==FALSE) {
    ...
    check_if_in_spoly <- function(the_poly, compare_to, the_proj = projection(main_shape)) {
    ...
    projection(spoly_spdf) <- projection(main_shape)
...
#'   file path to an RDS file OR a short name for the administrative shape file e.g., "DATE" or "current".
#'
...
#'
#' 2. \code{link_table} is a String referring to relase of admin shapefiles ("current" or e.g., "DATE"). In this case
...
build_simple_raster_pop <- function(subset_shape,
  ...
  pop_rast <- brick(paste0("FILEPATH"))
...
pull_custom_modeling_regions <- function(custom_regions){
  ...
  ref_reg_list <- list(
    ...
    #########################################################################################
    #### to add back once link table is working again!
    #########################################################################################
...
#'
set_root<-function(){
  root<<-ifelse(Sys.info()[1]=="Windows", "FILEPATH", "FILEPATH") # Setting "root" as global variable
}
