...
#'   qlogin. For more info, see USERNAME great writeup on the subject at URLPATH
...
#' @param core_repo (default `"FILEPATH"`) The directory
...
graph_data_coverage_values <- function(df,
  ...
  ## Define root
  assign("j_root", "FILEPATH", envir = .GlobalEnv)
  ...
  message(paste0("Note: At this point, all data and shapefiles have been loaded ",
                 ...
                 "message USERNAME or USERNAME")
  )
  
  # 2. Load main and disputed shapefiles ------------------------------------>
  
  # This takes a while, so let's do it once only
  if (!("main_shape_all" %in% ls())) {
    message("\nFast-loading main shapefile... ")
    assign(
      "main_shape_all",
      readRDS('FILEPATH'),
      envir = globalenv()
  ...
  if(save_on_share==TRUE) out_dir <- paste0('FILEPATH',
                                            indicator, extra_file_tag, '/'
  )
...
#'   with a filename in the folder 'FILEPATH'
...
get_country_list <- function(df, region, region_title) {
  ...
  #statement to deal with custom cases
    ...
    #create dictionary
    stage_main_list <- read.csv('FILEPATH')
    
    #create gaul to iso3 name dictionary
    iso3_names <- as.list(stage_main_list$iso3)
    iso3_names <- lapply(iso3_names, as.character)
    names(iso3_names) <- lapply(stage_main_list$GAUL_CODE, as.character)
...
#'
#' @description Pulls the polygons from a main shapefile based on input data.table.
#'
...
pull_polys <- function(shape_loc, fast_shapefiles) {
  ...
  # Read in the main shapefile
  if (fast_shapefiles == T) {
    main_shape <- fast_load_shapefile(shape)
  } else  {
    main_shape <- readOGR(dsn = paste0("FILEPATH"),
                            layer = shape)
  }
  
  message(paste0("    CRS: ",crs(main_shape)))
  
  names(main_shape)[names(main_shape)=="GAUL_Code"] <- "GAUL_CODE"
  
  # Subsetting will break if NAs in row index (GAUL_CODE)
  main_shape <- main_shape[!is.na(main_shape$GAUL_CODE),]
  
  # Custom fixes for broken shapefiles
  if (shape == "AZE_DHS_2006" & "GAUL_CODE" %in% names(main_shape) == F) {
    main_shape$GAUL_CODE <- main_shape$REGCODE
  }
  
  # Subset to the relevant data & shapefile bits (by gaul code)
  subset_shape <- main_shape[main_shape@data$GAUL_CODE %in% loc_codes, ]
...
dcp_check_for_missing_shapefiles <- function(df_graph_poly, fast_shapefiles) {
  ...
  if (fast_shapefiles){
    real_shapefiles <- gsub('.rds','',
                            tolower(list.files("FILEPATH",
                                               pattern = '.rds'))
    )
  } else {
    real_shapefiles <- gsub('.shp','',
                            tolower(list.files(paste0("FILEPATH"),
                                               pattern = '.shp'))
    )
  }
...
#'   "main_shape_all" in the code) and the disputed borders shapefile to a 
#'   central directory, "FILEPATH"
#' 
...
dcp_refresh_background_shps <- function(
  ...
  #  disputed shapefile in FILEPATH
...
build_g_data_scatter <- function(df_graph,
  ...
  # Note: 5/14 scalar from here: https://stackoverflow.com/questions/25061822/ggplot-geom-text-font-size-control
...
#'   (see Douglas-Peuker algorithm)
#' @param main_shape_all a combined SPDF with all polygons necessary to make maps
...
make_background_map <- function(region,
  ...
  background_map <- main_shape_all[main_shape_all@data$GAUL_CODE %in% gaul_list, ]
...
make_a_period_map <- function(period,
  ...
  # Merge polygons for this period to main spdf and add each one to map
...
prep_data_coverage_shiny <- function(df,
  ...
  save(list = 'just_polygons', file = paste0('FILEPATH', indicator, '_polygons.RData'))
  setnames(df, var, indicator)
  write.csv(df, paste0('FILEPATH', indicator, '_points.csv'))
