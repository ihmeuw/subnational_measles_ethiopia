...
#' is_singularity()
#' # TRUE only if container is spun up from image in "FILEPATH"
#' is_singularity(image_dir = "FILEPATH")
...
#' ---------|------------------------------------------------------------------
#'     A    | R in LBD image           | /FILEPATH
#'     B    | RStudio in LBD image     | FILEPATH
#'     C    | RStudio in non-LBD image | FILEPATH
#'

#' Note that R packages meant for use in RStudio not supported by the LBD team
#' were originally stored at 'FILEPATH'
#' but in October of 2018, were moved to
#' 'FILEPATH' after an update was made to the
#' cluster where FILEPATH was not necessarily accessible from all nodes.
...
#' (FILEPATH) and only has a limited number of packages
...
#' source_functions(paste0('/FILEPATH'))
#'
...
#' temporary directory to FILEPATH (a location
...
#'
#' @examples
#' mbg_setup(package_list = 'ggplot2', repos = 'FILEPATH')
