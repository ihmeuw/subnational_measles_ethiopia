...
#' @description A faster version of load_gbd_covariates. I've left the old one for backwards capability
...
load_gbd_covariates = function(covs, measures, year_ids, age_ids,
  ...
  shapefile_version <- 'DATE' ## to match the entries of world_shape
...
load_mbg_covariates <- function(mbg_fixed_effects, simple_polygon) {
  ...
  edu_0    <- brick('FILEPATH')
  edu_mean <- brick('FILEPATH')
...
save_standard_covariate <- function(cov_name,
  ...
  if (raked == T) {
    raster_file <- paste0("FILEPATH")
  } else if (raked == F) {
    raster_file_1 <- paste0("FILEPATH")
    raster_file_2 <- paste0("FILEPATHFILEPATH")
  ...
  message('\nAll layers successfully saved in FILEPATH')
...
#'  the "release" being a timestamp in the form of YYYY_MM_DD e.g., "DATE".
#'
#' @examples
...
#' @examples
#' \dontrun{
#' worldpop <- load_worldpop_covariate(simple_polygon, measure = 'a0004t', release = 'DATE')
#'
