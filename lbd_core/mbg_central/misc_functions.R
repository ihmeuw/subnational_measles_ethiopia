...
pullupstream<-function(){
  system('cd FILEPATH pull origin main')
}
...
waitformodelstofinishu5m<- function(sleeptime=100,
  ...
  paths <- list()
  if(ageasindic) {
    for(a in unique(lv[,2])) paths[[a]]<-paste0('FILEPATH')
  } else {
    paths[[1]] <-  paste0('FILEPATH')
  }
...
waitforresultstable <- function(st = strata,
  ...
  while(length(r_left) > 0 & length(u_left) > 0) {
    ...
    results_dir_r <- paste0('FILEPATH')
    results_dir_u <- paste0('FILEPATH')
...
#'   the default location of FILEPATH.
#'   [default = 'default']
#' 
...
make_qsub_share <- function(user = Sys.info()["user"],
  ...
  if (is.null(code)) {
    code <- sprintf("FILEPATH", corerepo)
  } else {
    code <- sprintf("FILEPATH", corerepo, ig, code)
  }
...
getPaths <- function(pathfile = 'FILEPATH') {
...
cleanup_inla_scratch <- function(run_date) {
  ...
  if(keep_inla_files==TRUE) {
    ...
    message(paste0('Files stored here: FILEPATH'))
...
plot_raster <- function(ras,
  ...
  #
  #     cores             = # of cores to use with mclapply
  #     individual_layers = plot each individual layer in its own png?
  #     fast_shapefiles   = do you want to pull RDS shapefiles?
  #
  #
  # Outputs:
  ...
  if (region == 'africa') {
    background_shp <- readOGR(dsn="FILEPATH", layer="africa_ad0")
  } else {
    background_shp <- readRDS("FILEPATH")
    background_shp <- subset(background_shp, ADM0_CODE %in% gaul_list)
  }
...
# Modified from USERNAME's code by USERNAME on DATE
# Revitalized by USERNAME on DATE
...
check_config <- function(cr = core_repo) {
  ...
  must_haves <- read.csv(paste0('FILEPATH'), header = F, stringsAsFactors = F)$V1
...
set_up_config <- function(repo, 
  ...
  if (is.null(config_name) & is.null(config_file)) {
    ...
    if (post_est_only == TRUE) 
      config <- data.table::fread(paste0("FILEPATH"))
  }
  ...
  if (.in.package()) {
    ...
    default_config_values <- data.table::fread(file.path('FILEPATH'), header = TRUE, stringsAsFactors = FALSE)
  }
  ...
  if(run_tests) {
    ...
    if (.in.package()) {
      ...
      config_tests <- data.table::fread(paste0('FILEPATH'), header = TRUE, stringsAsFactors = FALSE)
    }
...
proportional_area_map <- function(data, # Needs to have ADM0_CODE, ADM1_CODE, ADM2_CODE
  ...
  message("Loading main admin shape...")
...
#'                   'FILEPATH' (see \code{corerepo} below)
...
parallelize <- function(user = Sys.info()['user'],
  ...
  if(is.null(script_dir)) script_dir <- paste0('FILEPATH')
  ...
  #Set up region-specific memory settings for raking  
  if('region' %in% names(lv)){
    if(prefix %in% c('rake_fractional','unraked_agg','combine_dpt1','combine_dpt_dropout_cells_rasters','combine_dpt_under12',
                     'combine_dpt1_cells_rasters')){
      regional_raking_mem <- fread(paste0(indic_repo, 'profiling_raking.csv'))
    }
  }
  # Save objects ---------------------------------------------------------
  ...
  for (i in 1:nrow(lv)) {
    ...
    #Set region-specific memory for raking
    if('region' %in% names(lv)){
      if(prefix %in% c('rake_fractional','unraked_agg','combine_dpt1','combine_dpt_dropout_cells_rasters','combine_dpt_under12',
                       'combine_dpt1_cells_rasters')){    
        mem <- paste0(regional_raking_mem[region==lv[i,region]]$mem, 'G')
        resources[['m_mem_free']] <- mem    
      }
    }
...
monitor_jobs <- function(parallelize_output,
  ...
  if (keep_temp_file == F) unlink(paste0("FILEPATH"))
}
...
#' @examples
#' \dontrun{
#' # Note: this is within the CHILD SCRIPT, not the main script
#'
...
#' (\code{FILEPATH}) if a fork is being used.
#'
...
#'     'core_repo' is the default LBD core code main repo and will give
...
record_git_status <- function(core_repo,
  ...
  # if the user wants to make sure their repo is up-to-date with LBD main
  if(check_core_repo) {
    check_repo_report <- "\n********** REPO CHECK **********\n"
    # The two repo paths are the same
    if(clean_path(core_repo) == lbd_core_repo) {
      check_repo_report <- paste0(c(check_repo_report,
                                    "'core_repo' set to default LBD core code main repo: '",
                                    core_repo, "'\n"))
    } else {
      ...
      if(lbd_core_repo_hash == core_repo_hash) {
        check_repo_report <- paste0(c(check_repo_report,
                                      "Current 'core_repo' clone is up-to-date with LBD core code main repo:\n'",
                                      lbd_core_repo, "' == '", core_repo, "'\n"))
        # The two repo paths are not the same and the hash doesn't match, repos are out of sync
      } else {
        # print out the LBD core code main repo information for reference against current
        # repo being used as a warning.
        check_repo_report <- paste0(c(check_repo_report,
                                      "WARNING: Current 'core_repo' clone is out of sync with the LBD core code main repo:\n'",
                                      lbd_core_repo, "' != '", core_repo, "'\n",
                                      "\n\n** LBD Core Code main Repo Git Info **\n"))
        check_repo_report <- paste0(c(check_repo_report,
...
delete_model_outputs <- function(
  ...
  fileDF <- expand.grid(ind=indicator, ig=indicator_group, rd=run_date) %>%
    mutate(moddir=file.path('FILEPATH'))
...
#' @title Returns the stage main list
#'
