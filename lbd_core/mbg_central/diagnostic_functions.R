...
# Update: DATE Additional functions to be used with the
# FILEPATH script (USERNAME)
#**************************************************************************
...
plot_stackers <- function(reg,
  ...
  # Load main shape for outlines
  # main_shape <- readRDS('FILEPATH')
  main_shape <- readRDS(get_admin_shapefile(admin_level = 0, version = shapefile_version, suffix = ".rds"))
  main_shape <- subset(main_shape, ADM0_CODE %in% get_adm0_codes(reg, shapefile_version = shapefile_version))
  ...
  if (file.exists(paste0(mo_dir, ind, "_", reg, "_unraked_mean_raster.tif"))) {
    ...
    result_brick <- crop(result_brick, main_shape)
    result_brick <- mask(result_brick, main_shape)
  ...
  save_maps <- function(input_df, stacker_list, main_shape,
    ...
    if (!is.null(ctry)) {
      ...
      main_shape <- subset(main_shape, ADM0_CODE == get_adm0_codes(ctry, shapefile_version = shapefile_version))
      stacker_list <- lapply(stacker_list, function(x) {
        x <- suppressMessages(crop(x, extent(main_shape)))
        x <- suppressMessages(mask(x, main_shape))
        extent(x) <- extent(main_shape)
        return(x)
      })
    }

    main_shape_df <- suppressMessages(fortify(main_shape))
  ...
  save_maps(input_df, stacker_list, main_shape, result_brick, zmin,
  ...
  if (individual_countries == T) {
    ...
    gaul_list <- merge(gaul_list, gaul_to_loc_id, by = "GAUL_CODE")
     for (c in unique(gaul_list$ihme_lc_id)) {
      if (!(get_adm0_codes(c, shapefile_version = shapefile_version) %in% main_shape$ADM0_CODE)) {
        message(paste0("No shapes in main_shape corresponding to admin 0 code for ", c, " - skipping..."))
      } else {
        message(paste0("Saving stacker maps for country: ", c))
        save_maps(input_df, stacker_list, main_shape, result_brick,
...
#' if (interactive()) {
#'   get_data_estimate_df(,
#'   "child_growth_failure", "stunting_mod_b", "RUN_DATE", "wssa")
#' }
#' }
...
#' \dontrun{
#' if (interactive()) {
#'   read_inla_model("child_growth_failure", "stunting_mod_b", "RUN_DATE", "wssa")
#' }
#' }
...
#' \dontrun{
#' if (interactive()) {
#'   save_plot_list(plotList, "child_growth_failure", "stunting_mod_b", "RUN_DATE")
#' }
#' }
