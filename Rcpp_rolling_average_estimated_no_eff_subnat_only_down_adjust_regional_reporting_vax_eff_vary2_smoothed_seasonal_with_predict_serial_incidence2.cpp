...
// [[Rcpp::export]]
NumericMatrix predict_Likelihoodcpp_measles_transmission2_omp(NumericVector i_ticker_1, 
                                               ...
                                               NumericVector logit_rho1, 
                                               NumericVector logit_rho2, 
                                               NumericVector logit_rho3, 
                                               NumericVector logit_rho4, 
                                               NumericVector logit_rho5, 
                                               NumericVector logit_rho6, 
                                               NumericVector logit_rho7, 
                                               NumericVector logit_rho8, 
                                               NumericVector logit_rho9, 
                                               NumericVector logit_rho10, 
                                               NumericVector logit_rho11, 
  ...
  NumericMatrix to_export(2120* 79 * 24,10);
  ...
  rho1 = exp(logit_rho1[0]) / (1 + exp(logit_rho1[0]));
  rho2 = exp(logit_rho2[0]) / (1 + exp(logit_rho2[0]));
  rho3 = exp(logit_rho3[0]) / (1 + exp(logit_rho3[0]));
  rho4 = exp(logit_rho4[0]) / (1 + exp(logit_rho4[0]));
  rho5 = exp(logit_rho5[0]) / (1 + exp(logit_rho5[0]));
  rho6 = exp(logit_rho6[0]) / (1 + exp(logit_rho6[0]));
  rho7 = exp(logit_rho7[0]) / (1 + exp(logit_rho7[0]));
  rho8 = exp(logit_rho8[0]) / (1 + exp(logit_rho8[0]));
  rho9 = exp(logit_rho9[0]) / (1 + exp(logit_rho9[0]));
  rho10 = exp(logit_rho10[0]) / (1 + exp(logit_rho10[0]));
  rho11 = exp(logit_rho11[0]) / (1 + exp(logit_rho11[0]));
...
#pragma omp parallel
    ...
    for(int d2 = t_id3 * d_size3; d2 < (t_id3+1) * d_size3; d2 ++){    
      for(int a2 = t_id3 * a_size3; a2 < (t_id3+1) * a_size3; a2 ++){
        ...
        to_export(i + nobs_stratified*a2 + nobs_stratified*nage_groups*d2,0) = M_final_counts(a2,d2) ;
        to_export(i + nobs_stratified*a2 + nobs_stratified*nage_groups*d2,1) = S_final_counts(a2,d2)  ;
        to_export(i + nobs_stratified*a2 + nobs_stratified*nage_groups*d2,2) = I_final_counts(a2,d2)   ;
        to_export(i + nobs_stratified*a2 + nobs_stratified*nage_groups*d2,3) = R_final_counts(a2,d2) ;
        to_export(i + nobs_stratified*a2 + nobs_stratified*nage_groups*d2,4) = I_final_counts_INCIDENCE(a2,d2) ;
        
        
        
        to_export(i + nobs_stratified*a2 + nobs_stratified*nage_groups*d2,5) = foi(a2,d2);
