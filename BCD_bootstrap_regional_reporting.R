...
setwd('FILEPATH')
sourceCpp("FILEPATH/Rcpp_rolling_average_estimated_no_eff_subnat_only_down_adjust_regional_reporting_vax_eff_vary2_loess_seasonal_serial_interval_incidence_autocorrelation_nbinom5_regional_reporting_bootstrap_district.cpp")
...
sourceCpp('FILEPATH/Rcpp_BCD_rho_fitting_bootstrap_district_regional_reporting.cpp')
...
Likelihoodcpp_measles_transmission2_omp_short <- function(pars){
  ...
  liklihood_init <- Likelihoodcpp_measles_transmission2_omp(pop=pop, pop_array_vector=pop_array_vector,
                                                            ...
                                                            rho1=invlogit(logit_rho1_vals[i+1]),
                                                            rho2=invlogit(logit_rho2_vals[i+1]),
                                                            rho3=invlogit(logit_rho3_vals[i+1]),
                                                            rho4=invlogit(logit_rho4_vals[i+1]),
                                                            rho5=invlogit(logit_rho5_vals[i+1]),
                                                            rho6=invlogit(logit_rho6_vals[i+1]),
                                                            rho7=invlogit(logit_rho7_vals[i+1]),
                                                            rho8=invlogit(logit_rho8_vals[i+1]),
                                                            rho9=invlogit(logit_rho9_vals[i+1]),
                                                            rho10=invlogit(logit_rho10_vals[i+1]),
                                                            rho11=invlogit(logit_rho11_vals[i+1]),
...
Likelihood_reporting_short <- function(pars){
  ...
  -1 * likelihood_reporting_fit(rho1 = invlogit(pars[1]), 
                                rho2 = invlogit(pars[2]), 
                                rho3 = invlogit(pars[3]), 
                                rho4 = invlogit(pars[4]), 
                                rho5 = invlogit(pars[5]), 
                                rho6 = invlogit(pars[6]), 
                                rho7 = invlogit(pars[7]), 
                                rho8 = invlogit(pars[8]), 
                                rho9 = invlogit(pars[9]), 
                                rho10 = invlogit(pars[10]), 
                                rho11 = invlogit(pars[11]), 
...
logit_rho1_vals <- rep(NA, 11)
logit_rho2_vals <- rep(NA, 11)
logit_rho3_vals <- rep(NA, 11)
logit_rho4_vals <- rep(NA, 11)
logit_rho5_vals <- rep(NA, 11)
logit_rho6_vals <- rep(NA, 11)
logit_rho7_vals <- rep(NA, 11)
logit_rho8_vals <- rep(NA, 11)
logit_rho9_vals <- rep(NA, 11)
logit_rho10_vals <- rep(NA, 11)
logit_rho11_vals <- rep(NA, 11)
...
logit_rho8_vals[1] <- -5
logit_rho9_vals[1] <- -5
logit_rho10_vals[1] <- -5
logit_rho11_vals[1] <- -5
...
for(i in 1:10){
  ...
  prediction_measles_cases <- predict_Likelihoodcpp_measles_transmission2_omp(i_ticker_1 = i_ticker_1,
                                                                              ...
                                                                              logit_rho1 = logit_rho1_vals[i], 
                                                                              logit_rho2 = logit_rho2_vals[i], 
                                                                              logit_rho3 = logit_rho3_vals[i], 
                                                                              logit_rho4 = logit_rho4_vals[i], 
                                                                              logit_rho5 = logit_rho5_vals[i], 
                                                                              logit_rho6 = logit_rho6_vals[i], 
                                                                              logit_rho7 = logit_rho7_vals[i], 
                                                                              logit_rho8 = logit_rho8_vals[i], 
                                                                              logit_rho9 = logit_rho9_vals[i], 
                                                                              logit_rho10 = logit_rho10_vals[i], 
                                                                              logit_rho11 = logit_rho11_vals[i], 
  ...
  model_fit_object_new_rho <- nlm(Likelihood_reporting_short, c(logit_rho1_vals[i],
                                                                logit_rho2_vals[i], 
                                                                logit_rho3_vals[i], 
                                                                logit_rho4_vals[i], 
                                                                logit_rho5_vals[i], 
  ...
  logit_rho1_vals[i+1] <- model_fit_object_new_rho$estimate[1]
  logit_rho2_vals[i+1] <- model_fit_object_new_rho$estimate[2]
  logit_rho3_vals[i+1] <- model_fit_object_new_rho$estimate[3]
  logit_rho4_vals[i+1] <- model_fit_object_new_rho$estimate[4]
  logit_rho5_vals[i+1] <- model_fit_object_new_rho$estimate[5]
  logit_rho6_vals[i+1] <- model_fit_object_new_rho$estimate[6]
  logit_rho7_vals[i+1] <- model_fit_object_new_rho$estimate[7]
  logit_rho8_vals[i+1] <- model_fit_object_new_rho$estimate[8]
  logit_rho9_vals[i+1] <- model_fit_object_new_rho$estimate[9]
  logit_rho10_vals[i+1] <- model_fit_object_new_rho$estimate[10]
  logit_rho11_vals[i+1] <- model_fit_object_new_rho$estimate[11]
  ...
  if(i == 10){
    ...
    prediction_measles_cases <- predict_Likelihoodcpp_measles_transmission2_omp(i_ticker_1 = i_ticker_1,
                                                                                ...
                                                                                logit_rho1 = logit_rho1_vals[i+1], 
                                                                                logit_rho2 = logit_rho2_vals[i+1], 
                                                                                logit_rho3 = logit_rho3_vals[i+1], 
                                                                                logit_rho4 = logit_rho4_vals[i+1], 
                                                                                logit_rho5 = logit_rho5_vals[i+1], 
                                                                                logit_rho6 = logit_rho6_vals[i+1], 
                                                                                logit_rho7 = logit_rho7_vals[i+1], 
                                                                                logit_rho8 = logit_rho8_vals[i+1], 
                                                                                logit_rho9 = logit_rho9_vals[i+1], 
                                                                                logit_rho10 = logit_rho10_vals[i+1], 
                                                                                logit_rho11 = logit_rho11_vals[i+1], 
  ...
  save(beta_max_vals, beta_min_vals,
       logit_rho1_vals,  logit_rho2_vals,  logit_rho3_vals,  logit_rho4_vals,  logit_rho5_vals,  logit_rho6_vals,  logit_rho7_vals,  logit_rho8_vals, 
       logit_rho9_vals,  logit_rho10_vals,  logit_rho11_vals, 
       beta_val_ll, rho_val_ll, 
       I_predicted, file = paste0('FILEPATH/',run_date,'/bootstrap_',boot_i,'_BCD_iteration_', i, '.RData') )
