...
message("Loading Data...")
load(paste0(ratio_dir, "dpt3_timeliness_ratio_raked_cell_draws_eb_bin0_", reg, "_0.RData"))

ratio_cell_pred <- copy(raked_cell_pred)


## COMBINE CELL PREDS ######################################################################################

cell_pred <- dpt3_cell_pred * ratio_cell_pred
...
save(cell_pred, file = paste0(
  under12_dir, "dpt3_cov_under12_raked_cell_draws_eb_bin0_", reg, "_0.RData"
))
...
save_cell_pred_summary <- function(summstat, raked, ...) {
  ...
  if (raked == "raked_c") {
    ...
    load(paste0(sharedir, "/output/", run_date_ratio, "/", indicator, "_raked_c_cell_draws_eb_bin0_", reg, "_0.RData" ))
    mask_raster <- "raked_simple_raster"
  }
...
# })
  ...
  setwd("FILEPATH")
  load(paste0(dpt3_run_date, "_bin0_", reg, "_0.RData"))
  # setwd("FILEPATH")
